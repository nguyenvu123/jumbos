...
function tvlgiao_wpdance_loop_price_contact_to_us(){
	?>
		<div class="wd-contact-to-us">
			<span><?php esc_html_e('Giá liên hệ','wpdance'); ?></span>
		</div>
	<?php
}
function tvlgiao_single_price_contact_to_us(){
	$telephone_number = get_theme_mod('tvlgiao_wpdance_telephone_number','0936.987.882');
	?>
		<div class="wd-single-price-us">
			<p><?php esc_html_e('Gọi để báo giá: ','wpdance'); ?><span><?php esc_html_e($telephone_number);?></span></p>
		</div>
	<?php
}
...
function tvlgiao_wpdance_upsell_related_display(){ ?>
	<div class="wd-ralated-product">
		<h5 class="wd-heading-title"><?php esc_html_e('RELATED ITEMS','wpdance'); ?></h5>
...
function tvlgiao_wpdance_customize_product_config(){
    ...
    if($show_price == "no"){
    	...
    	add_action('woocommerce_after_shop_loop_item_title','tvlgiao_wpdance_loop_price_contact_to_us',5);
    	remove_action('woocommerce_single_product_summary','woocommerce_template_single_price',14);
    	add_action('woocommerce_single_product_summary','tvlgiao_single_price_contact_to_us',14);
    }
